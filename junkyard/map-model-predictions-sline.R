...
slinesf<-st_transform(slinesf, crs = 4326)
# plot(slinesf)

names(slinesf)<-c("r_node_name", "r_node_n_type", "corridor_route", "corridor_direction", "corridor_name","corridor_conflated_index", "geometry")
...
diffs_sline <- merge(slinesf, diffs, all.x = F, all.y = F)
# 298607

str(diffs_sline)

test <- sf::st_as_sfc(diffs_sline)
...
leaflet(diffs_sline)%>%
  ...
  addPolygons(data = diffs_sline[diffs_sline$date == '2020-03-21' ,],
              color = ~binpal(volume.diff), 
              # stroke = T, 
              fillOpacity = 0.75, 
              # radius = ~5*(scl_volume),
              label = node_labels[diffs_sline$date == '2020-03-21' ],
              group = "Sat. March 21")
